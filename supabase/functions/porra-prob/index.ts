...
import { DATA, ENGINE } from "https://cdn.jsdelivr.net/gh/pablomarin21/__REPO__@__COMMIT__/engine-esm.js";
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    const { data: pool } = await sb.from("porra_pools").select("id,locked,lock_at,settings").eq("code", code).maybeSingle();
    if (!pool) return jsonR({ error: "POOL_NOT_FOUND" }, 404);
    const { data: parts } = await sb.from("porra_participants").select("id,first_name,last_name,created_at").eq("pool_id", pool.id).order("created_at");
    const { data: preds } = await sb.from("porra_predictions").select("participant_id,picks").eq("pool_id", pool.id);
    const picksById: Record<string, any> = {};
    (preds || []).forEach((p: any) => { picksById[p.participant_id] = p.picks; });
    const { data: results } = await sb.from("porra_results").select("*");
    const dbResults: Record<string, any> = {};
    (results || []).forEach((r: any) => { dbResults[r.match_code] = r; });
    const { data: cfg } = await sb.from("porra_config").select("extras_actual").eq("id", 1).maybeSingle();
